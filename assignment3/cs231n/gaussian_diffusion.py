...
class GaussianDiffusion(nn.Module):
    ...
    def predict_start_from_noise(self, x_t, t, noise):
        ...
        one_sqrt_a_bar = extract(self.sqrt_one_minus_alphas_cumprod, t, x_t.shape)
        x_start = (x_t - one_sqrt_a_bar * noise) / sqrt_a_bar
        ####################################################################
        return x_start
    ...
    def predict_noise_from_start(self, x_t, t, x_start):
        ...
        sqrt_a_bar = extract(self.sqrt_alphas_cumprod, t, x_t.shape)
        one_sqrt_a_bar = extract(self.sqrt_one_minus_alphas_cumprod, t, x_t.shape)
        pred_noise = (x_t - sqrt_a_bar * x_start) / one_sqrt_a_bar
        ####################################################################
        return pred_noise
    ...
    def p_sample(self, x_t, t: int, model_kwargs={}):
        ...
        x_start = torch.clamp(model_out, -1, 1)
    ...
    def q_sample(self, x_start, t, noise):
        ...
        one_sqrt_a_bar = extract(self.sqrt_one_minus_alphas_cumprod, t, x_start.shape)
        x_t = sqrt_a_bar * x_start + one_sqrt_a_bar * noise
        ####################################################################
        return x_t
    ...
    def p_losses(self, x_start, model_kwargs={}):
        ...
        model_out = self.model(x_t, t, model_kwargs)
        raw_loss = nn.functional.mse_loss(model_out, target, reduction='none')
        loss = (loss_weight * raw_loss).mean()
        ####################################################################
