...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for i in range(2*N):
            if i != k:
                divider1 += torch.exp(sim(z_k, out[i])/tau)
            if i != k+N:
                divider2 += torch.exp(sim(z_k_N, out[i])/tau)
        l1 = -torch.log(torch.exp(sim(z_k, z_k_N)/tau) / divider1)
        l2 = -torch.log(torch.exp(sim(z_k, z_k_N)/tau) / divider2)
        total_loss += l1 + l2
...
def compute_sim_matrix(out):
    ...
    sim_matrix = torch.matmul(out, out.T) / torch.matmul(norm, norm.T)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    exponential_clone = exponential.clone()
    exponential_clone.fill_diagonal_(0)
    denominator = exponential_clone.sum(dim=1,keepdim=True)
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    numerator = torch.exp(torch.cat([sim_pairs,sim_pairs], dim=0) / tau)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    # Step 4: Now that you have the numerator and denominator for all augmented samples, compute the total loss.
    loss = -torch.log(numerator/denominator).sum(dim=0) / (2 * N)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
