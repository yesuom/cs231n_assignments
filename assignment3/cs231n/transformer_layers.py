...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        j = torch.arange(0, embed_dim, step=2).float()
        j = torch.exp((-j/embed_dim)*math.log(10000))
        angles = position * j
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        ############################################################################
        query = self.query(query).reshape((N, S, self.n_head, self.head_dim)).permute(0,2,1,3) #(N,H,S,DH)
        key_T = self.key(key).reshape((N, T, self.n_head, self.head_dim)).permute(0,2,3,1) #K_T: (N,H,DH,T)
        value = self.value(value).reshape((N, T, self.n_head, self.head_dim)).permute(0,2,1,3)
        logits = torch.matmul(query, key_T)/math.sqrt(self.head_dim)
        if attn_mask is not None:
            logits = logits.masked_fill(attn_mask==0, float('-inf'))
        # att_weight = self.attn_drop(torch.softmax(logits, dim=-1))
        att_weight = torch.softmax(logits, dim=-1)
        att_out = torch.matmul(att_weight, value)
        att_out = self.attn_drop(att_out)
        att_out = att_out.permute(0,2,1,3).reshape(N, S, E)
        output = self.proj(att_out)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return output
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 2, 4, 1, 3, 5) # (N, h, w, C, P, P)
        x = x.reshape(N, (H//self.patch_size)*(H//self.patch_size), C*self.patch_size*self.patch_size)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, src_mask=None):
        ...
        ############################################################################
        shortcut = src
        tgt = self.self_attn(query=src, key=src, value=src, attn_mask=src_mask)
        tgt = self.dropout_self(tgt)
        tgt = tgt + shortcut
        tgt = self.norm_self(tgt)
