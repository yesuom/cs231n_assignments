...
class Unet(nn.Module):
    ...
    def cfg_forward(self, x, time, model_kwargs={}):
        ...
        model_kwargs_un = {"text_emb": None}
        x_uncond = self.forward(x, time, model_kwargs_un)
    ...
    def forward(self, x, time, model_kwargs={}):
        ...
        ##################################################################
        skip_list = []
        for resnet1, resnet2, downsampler in self.downs:
            x = resnet1(x, context)
            skip_list.append(x)
            x = resnet2(x, context)
            skip_list.append(x)
        ...
        for upsampler, resnet1, resnet2 in self.ups:
            x = upsampler(x)
            x = resnet1(torch.cat((x, skip_list.pop()),dim=1), context)
            x = resnet2(torch.cat((x, skip_list.pop()),dim=1), context)
        ##################################################################
