...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        loss = temporal_softmax_loss(scores, captions_out.long(), mask, verbose=False)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def sample(self, features, max_length=30):
        ...
        ###########################################################################
        captions[:,0] = self._start * torch.ones(N, dtype=torch.long)
        h = features @ W_proj + b_proj
        for i in range(1, max_length):
            x_pre = word_embedding_forward(captions[:,i-1], W_embed)
            h = rnn_step_forward(x_pre, h, Wx, Wh, b)
            x = h @ W_vocab + b_vocab
